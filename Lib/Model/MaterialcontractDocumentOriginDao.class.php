<?php
	class MaterialcontractDocumentOriginDao extends Model{
		...
		public function findById($documentid){
			...
			$myPDO = new PDO($dsn,C('MYSQL_USERNAME'),C('MYSQL_PASSWORD'));
			$myPDO->query('set names utf8;'); 
			$sql="select * from tb_materialcontract_document_origin where documentid=?";
		...
		public function findByContractid($contractid){
			...
			$myPDO = new PDO($dsn,C('MYSQL_USERNAME'),C('MYSQL_PASSWORD'));
			$myPDO->query('set names utf8;'); 
			$sql="select * from tb_materialcontract_document_origin where contractid=?";
			$statement = $myPDO->prepare($sql);
		...
		public function add($contractid,$name,$path,$remark){
			...
			$myPDO = new PDO($dsn,C('MYSQL_USERNAME'),C('MYSQL_PASSWORD'));
			$myPDO->query('set names utf8;');
			$sql="insert into tb_materialcontract_document_origin(contractid,name,path,remark) values(?,?,?,?)";
			$statement = $myPDO->prepare($sql);
		...
		public function updateById($documentid,$contractid,$name,$path,$remark){
			...
			$myPDO = new PDO($dsn,C('MYSQL_USERNAME'),C('MYSQL_PASSWORD'));
			$myPDO->query('set names utf8;');
			$sql="update tb_materialcontract_document_origin set contractid=?,name=?,path=?,remark=? where documentid=?";
			$statement = $myPDO->prepare($sql);
		...
		public function deleteById($documentid){
			...
			$myPDO = new PDO($dsn,C('MYSQL_USERNAME'),C('MYSQL_PASSWORD'));
			$myPDO->query('set names utf8;');
			$sql="delete from tb_materialcontract_document_origin where documentid=?";
			$statement = $myPDO->prepare($sql);
