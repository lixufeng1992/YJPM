<?php
	class SubcontractDocumentDao extends Model{
		public function findAll(){
			$dsn = C('DB_DSN1');
			$myPDO = new PDO($dsn,C('MYSQL_USERNAME'),C('MYSQL_PASSWORD'));
			$myPDO->query('set names utf8;'); 
			$sql="select * from tb_subcontract_document order by documentid asc";
		...
		public function findById($documentid){
			...
			$myPDO = new PDO($dsn,C('MYSQL_USERNAME'),C('MYSQL_PASSWORD'));
			$myPDO->query('set names utf8;'); 
			$sql="select * from tb_subcontract_document where documentid=?";
		...
		public function findByContractid($contractid){
			...
			$myPDO = new PDO($dsn,C('MYSQL_USERNAME'),C('MYSQL_PASSWORD'));
			$myPDO->query('set names utf8;'); 
			$sql="select * from tb_subcontract_document where contractid=?";
			$statement = $myPDO->prepare($sql);
		...
		public function add($contractid,$name,$path,$remark,$create_userid,$lastupdate_userid,$checked_userid){
			...
			$myPDO = new PDO($dsn,C('MYSQL_USERNAME'),C('MYSQL_PASSWORD'));
			$myPDO->query('set names utf8;');
			$sql="insert into tb_subcontract_document(contractid,name,path,remark,create_userid,lastupdate_userid,checked_userid) values(?,?,?,?,?,?,?)";
			$statement = $myPDO->prepare($sql);
		...
		public function updateById($documentid,$contractid,$name,$path,$remark,$create_userid,$lastupdate_userid,$checked_userid){
			...
			$myPDO = new PDO($dsn,C('MYSQL_USERNAME'),C('MYSQL_PASSWORD'));
			$myPDO->query('set names utf8;');
			$sql="update tb_subcontract_document set contractid=?,name=?,path=?,remark=?,create_userid=?,lastupdate_userid=?,checked_userid=? where documentid=?";
			$statement = $myPDO->prepare($sql);
		...
		public function deleteById($documentid){
			...
			$myPDO = new PDO($dsn,C('MYSQL_USERNAME'),C('MYSQL_PASSWORD'));
			$myPDO->query('set names utf8;');
			$sql="delete from tb_subcontract_document where documentid=?";
			$statement = $myPDO->prepare($sql);
