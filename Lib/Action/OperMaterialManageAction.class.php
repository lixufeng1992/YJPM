...
class OperMaterialManageAction extends LoginAfterAction
{
    ...
    public function addContractSubmit()
    {
        ...
        if (isset($_POST['check_grossamount_control']))
            $check_grossamount_control = 1;
        else
            $check_grossamount_control = 0;
    ...
    public function editContract()
    {
        ...
        foreach ($contractContentRowArray as $key => $value) {
            ...
            if (empty($value['plan_id']) == false) {
                ...
                $contractContentDisplayRowArray[$index]['plan_number'] = "";
            $index ++;
        }
    ...
    public function editContractSubmit()
    {
        ...
        $result = $this->materialcontractDao->updateById($contractid, $resource_id, $contract_number, $name, $a_part_enterpriseid, $supplier_enterpriseid, $duty_officer, $companyid, $departmentid, $build_pattern, $sign_date, $pay_baseprice_according, $margin_amount, $pay_limit, $check_grossamount_control, $main_content, $other_limit_condition, $remark, $contractRow['finalcost_userid'], $contractRow['check_userid'], $contractRow['create_userid'], $contractRow['create_date']);
    ...
    public function editContract_uploadfile()
    {
        ...
        $returnData['download_url'] = U('OperMaterialManage/editContract_downloadDocument', array(
    ...
    public function editContract_uploadfileOrigin()
    {
        ...
        $returnData['doc_remark'] = $doc_remark;
        $returnData['suffix'] = $suffix;
        $returnData['download_url'] = U('OperMaterialManage/editContract_downloadDocumentOrigin', array(
            'documentid' => $returnData['documentid']
        ));
    ...
    public function editOutboundOrder()
    {
        ...
        foreach ($materialInventoryUnselectedRowArray as $key => $value) {
            ...
            if ($value['warehouse_id'] == $warehouse_id) {
                ...
                $materialInventoryUnsameWarehouseRowArray[] = $materialInventoryUnselectedRowArray[$key];
        }
